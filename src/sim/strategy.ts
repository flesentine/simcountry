...
export function countryLogistics(world: WorldState, countryId: string) {
  ...
  return clamp(0.48 + averageQuality * 0.18 - blockadePenalty, 0.25, 1.35);
}
...
export function calculateWarSupply(world: WorldState, country: Country, opponentId: string) {
  ...
  return clamp(22 + stock + fiscal + logistics + landAccess, 8, 100);
}
...
export function runInfrastructure(world: WorldState, rng: RngLike) {
  ...
  for (const country of world.countries) {
    ...
    if (rng.next() > 0.18 + country.policy.commerce / 150) continue;
    const candidates = world.geography.routes
    ...
    const cost = upgrade ? 7 + route.level * 5 : 3.5 + (100 - route.condition) * 0.04;
    ...
    recalculateRouteCapacity(route);
    messages.push(`${country.name} invests $${round(cost)}B in ${route.id}, raising it to ${route.infrastructure} level ${route.level} at ${Math.round(route.condition)}% condition.`);
  }
  return messages;
}
...
export function runAnnualDemography(world: WorldState) {
  ...
  for (const city of world.geography.cities) {
    ...
    const annualGrowth = clamp(
      0.004 + (country.stability - 50) / 4000 + (city.port ? 0.004 : 0) + infra * 0.0015 - (atWar ? 0.018 : 0),
      -0.035,
      0.035,
    );
  ...
  const sources = [...world.countries]
    .filter((country) => country.stability < 52 || world.wars.some((war) => war.a === country.id || war.b === country.id))
    .sort((a, b) => a.stability - b.stability);

  for (const source of sources) {
    const destinations = world.countries
      .filter((target) => target.id !== source.id && target.stability > source.stability + 12)
    ...
    if (!target) continue;
    const amount = round(Math.min(0.28, source.population * 0.0018, Math.max(0, (target.stability - source.stability) * 0.006)));
